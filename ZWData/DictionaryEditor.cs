...
internal class DicitonaryEditor : CollectionEditor {
    ...
    private static object GetStringInt(object[] value) {
        ...
        for (int i = 0; i < value.Length; i++) {
            ...
            string[] ss = s.Split(new string[] { ", " }, StringSplitOptions.None);
            if (ss.Length < 2) continue;
            if (int.TryParse(ss[1], out int dvalue)) {
    ...
    private static object GetIntString(object[] value) {
        ...
        for (int i = 0; i < value.Length; i++) {
            ...
            string[] ss = s.Split(new string[] { ", " }, StringSplitOptions.None);
            if (ss.Length < 2) continue;
            if (int.TryParse(ss[0], out int dkey)) {
                pairList[dkey] = ss[1];
            }
        }
        return pairList;
    ...
    private static object GetIntInt(object[] value) {
        ...
        for (int i = 0; i < value.Length; i++) {
            ...
            string[] ss = s.Split(new string[] { ", " }, StringSplitOptions.None);
            if (ss.Length < 2) continue;
            if (int.TryParse(ss[0], out int dkey) && int.TryParse(ss[1], out int dvalue)) {
